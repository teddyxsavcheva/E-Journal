...
const School = () => {
    ...
    useEffect(() => {
        const getAverage = async () => {
            try {
                const response = await axios.get(`/headmaster/${headmasterId}/avg-grade-for-school`);
                if (response.data && response.data.length > 0) {
                    const averageValue = response.data[0].split(',')[1].trim();
                    setAverage(averageValue);
                }
            } catch (error) {
                setError(error);
                console.error('There was an error getting the average!', error);
            }
        };
